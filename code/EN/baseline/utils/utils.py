    ...
    for line_no, i in enumerate(summ1):
        if "-" in i:
            ...
            for k1 in range(rng):
                ...
                if st1:
                    summ1[line_no + k1 + 1] = summ1[line_no + k1 + 1].replace(
                        st1, "They"
                    )
                    summ1[line_no + k1 + 1] = summ1[line_no + k1 + 1].replace(
                        "They's", "Their"
...
def gen_tscs(test_dataset, tokenizer, length):
    with open("../{}.json".format(test_dataset), "r") as out:
        tscs = json.load(out)
...
def gen_tsc(tsc, tokenizer, length):
    meeting_id, roles, utterances = (
        tsc["meeting_id"],
        tsc["roles"],
        tsc["utterances"],
    )
...
def gen_summary(tscs_preprocessed, model, tokenizer, outdir):
    s2 = []
    filename = []
    meet_no = 1
    for tsc in tscs_preprocessed:
        ...
        for i1, t1 in enumerate(tsc):
            inputs = tokenizer(t1, return_tensors="pt").to(device)
        ...
        #     os.mkdir(outdir)
        #
        # outpath = '{}/{}.txt'.format(outdir, k)
        # outfile = open(outpath, 'w')
        # outfile.write(s1)
        # outfile.close()
        # print('Minute saved at {}'.format(outpath))
        # meet_no+=1
